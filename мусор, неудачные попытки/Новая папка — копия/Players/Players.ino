...
void setup() {
  ...
  radio.openWritingPipe(address[numPlayer - 1]);
  radio.powerUp();
  radio.stopListening();
...
void loop() {
  ...
  if (!butt && !flag) {
    ...
    if (radio.write(&myData, sizeof(myData))) {
      Serial.print("Кноп отправка "); Serial.println(myData[0]);
    } else Serial.println("Кноп феил отправки");

    if (radio.isAckPayloadAvailable()) {
      radio.read(&ackData, sizeof(ackData));
      if (ackData[0] == numPlayer * 11) win();
      Serial.print("Кноп обратная связь "); Serial.println(ackData[0]);
    } else Serial.println("Кноп феил обратной связи");
  }

  if (millis() - timing > 140 + numPlayer * 6) {
